...
\begin{align*}
  ...
  \\
  &\text{extra thermosyphon:}\\
  &k_t(h(z_t, t) - T_{t}(z_t, t))\delta(z - z_t) = \int_\Omega\psi_i k_t(h - T_{t})\delta(z - z_t)dz = \int_\Omega\psi_i k_t(\sum_{j=1}^N\hat{h}\psi_j - T_{t})\delta(z - z_t)dz\\
  &= (k_t\hat{u} - T_t)_i&
  \\
